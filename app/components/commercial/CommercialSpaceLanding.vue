...
export type CommercialNavLink = { label: string; to: string }
...
const props = defineProps<{
  ...
  subtitle: string
  searchIntro: string
  navLinks: CommercialNavLink[]
  pillarsSectionEyebrow: string
  pillarsHeading: string
  pillarsLead: string
  pillars: CommercialPillar[]
  previewKicker: string
  previewTitleBefore: string
  previewSubtitle: string
  noPreviewMessage: string
  checklistTitle: string
  checklistLead: string
  checklist: string[]
  listCtaEyebrow: string
  listCtaTitle: string
  listCtaBody: string
  /** When set and URL has no `type` query, pre-fills space type (e.g. `office`). */
...
const {
  ...
  syncFromRoute: true,
  defaultIntentFromRoute: 'commercial',
  lockedIntent: 'commercial',
})

onMounted(() => {
  const qType = typeof route.query.type === 'string' ? route.query.type : ''
  if (qType) return
  const p = props.presetPropertyType
  if (p) propertyType.value = p
})

const heroBgStyle = computed(() => ({
  background: intentHeroBackgrounds.commercial,
}))
...
watch(
  [city, propertyType, budget, locality],
  () => {
    void resetAndLoad()
  },
  { immediate: true },
...
const localityInputId = computed(() => `${props.inputIdPrefix}-locality-input`)
</script>
  ...
  <div class="w-full overflow-x-hidden">
    ...
    >
      ...
      >
        ...
        />
        <HeroLineArtBackdrop intent="commercial" class="absolute inset-0 z-[1]" />
      </div>
      ...
      >
        <div class="mx-auto w-full max-w-3xl">
          ...
          <p class="mt-3 flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-center text-xs text-white/60">
          ...
          >
            ...
            <div class="p-3.5 sm:p-5 md:p-6">
              ...
              >
                {{ searchIntro }} {{ cityLabel }}
              </p>

              <div class="mt-1">
                <label class="sr-only" :for="localityInputId">Area or landmark</label>
                <div
                ...
                >
                  ...
                  >
                    ...
                    <input
                      ...
                      placeholder="Tech park, CBD, high street…"
                      autocomplete="off"
